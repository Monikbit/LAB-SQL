...
# ACA INTENTE QUE ME LISTARA EL ACTOR CON MAS ACTUACIONES POR PELICULA PERO NO LO LOGRE
# No super como Eliminar los registros que no eran el max(num) por actor  en cada pelicula
# y si lo piensas con un self join donde titulo o IDpeli seas diferentes 
# IDEA SELF JOIN DONDE NUM SEA MAYOR DEL LADO IZQUIERDO
SELECT f.film_id AS IDPeli, f.title AS Titulo ,fa.actor_id as IDActor , CONCAT(a.first_name,' ' , a.last_name) AS Nombre,  COUNT(f.film_id)  OVER (PARTITION BY fa.actor_id  )as Num
FROM film_actor AS fa JOIN actor AS a ON fa.actor_id = a.actor_id
